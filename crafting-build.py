...
def merge_folders(folder1, folder2, folder3, output_folder):
    ...
    # Copy all contents of folder3 to the output folder
    for item in os.listdir(folder3):
        item_path = os.path.join(folder3, item)
        if os.path.isdir(item_path):
            shutil.copytree(item_path, os.path.join(output_folder, item), dirs_exist_ok=True)
        else:
            shutil.copy2(item_path, output_folder)
...
def main():
    ...
    # Ensure the 'crafting-common' folder exists
    if not os.path.exists(folder3):
        print(f"Error: 'crafting-common' folder not found in {script_dir}")
        return
    ...
    try:
        ...
        merge_folders(folder1, folder2, folder3, temp_folder)
